...
public static class MainClass
{
    ...
    private const string STORAGE = "gdrive_uploader";
    ...
    public static void DeleteCachedRefreshToken()
    {
        AuthorizationMgr.DeleteCachedRefreshToken(STORAGE);
    }
    ...
    public static List<File> AuthorizeAndListDirectoryFiles(string DirectoryName)
    {
        ...
        // Get a listing of the existing files...
        List<File> allFiles = GoogleDrive.Util.Utilities.RetrieveAllFiles(_service);
        List<File> fileList = GoogleDrive.Util.Utilities.RetrieveDirectoryFiles(_service, DirectoryName);
        List<File> ExitesFolders = new List<File>();
        List<File> FilterFiles = new List<File>();
        foreach (File item in fileList)
        {
            if (item.MimeType != "application/vnd.google-apps.folder")
                continue;
            if (item.Labels.Trashed.HasValue)
                if ((bool)item.Labels.Trashed.Value)
                    continue;

            ExitesFolders.Add(item);
        }
        foreach (File item_parent in ExitesFolders)
        {
            foreach(File item_child in allFiles)
            {
                if (item_child.MimeType == "application/vnd.google-apps.folder")
                    continue;

                //if (isFileInFolder(_service, item_parent.Id, item_child.Id))
                if (item_child.Parents.Count == 0)
                    continue;

                if (item_child.Labels.Trashed.HasValue)
                    if ((bool)item_child.Labels.Trashed.Value)
                        continue;

                foreach (ParentReference parent in item_child.Parents)
                {
                    if (parent.Id == item_parent.Id)
                        FilterFiles.Add(item_child);
                }
            }
        }
        return FilterFiles;
    ...
    public static bool isFileInFolder(DriveService service, string folderId, string fileId)
    {
        try
        {
            service.Children.Get(folderId, fileId).Fetch();
        }
